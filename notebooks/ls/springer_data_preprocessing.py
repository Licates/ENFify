...
# Paths
FEATURE_PATH = Path(PROCESSED_DATA_DIR / "Synthetic_40s_CNNBiLSTM")
os.makedirs(FEATURE_PATH, exist_ok=True)


# Uncut
uncut_files = Path(RAW_DATA_DIR / "synthetic" / "40s" / "uncut" / "*.wav")
uncut_files = sorted(glob(str(uncut_files)))
uncut_spatial = []
uncut_temporal = []

# Cut
cut_files = Path(RAW_DATA_DIR / "synthetic" / "40s" / "cut" / "*.wav")
...
# Save the data
np.save(str(FEATURE_PATH) + "/" + "synth_temporal_phases_40s.npy", temporal_data)
np.save(str(FEATURE_PATH) + "/" + "synth_spatial_phases_40s.npy", spatial_data)
np.save(str(FEATURE_PATH) + "/" + "synth_phases_40s_labels.npy", labels)
